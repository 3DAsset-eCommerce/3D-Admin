...
export default function TextInput({type='input' ,width, height=4.2, placeholder, inputValue,counter, disabled=false, assetdata, setAssetdata}:TextInputProps) {
  ...
  const handlerCountInput = (e:ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    ...
    {type === 'input' ? (
        <form onSubmit={handlerSubmit}>
          <input style={{width: `${width}rem`, height:`${height}rem`}} className='py-[0.9rem] rounded border border-[#474E57] bg-neutral-navy-950 px-[2rem] text-[1.4rem] focus:border-neutral-navy-200' ref={inputRef} placeholder={inputValue ? inputValue : placeholder}  disabled={disabled} onChange={handlerCountInput}/>
